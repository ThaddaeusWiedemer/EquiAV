    ...
    def __init__(
        ...
        **kwargs
        ...
        if inter_linear:
            ...
            self.pred_a_inter = MLPProjectionHead(
                in_dim=embed_dim, hidden_dim=proj_hidden_dim, out_dim=proj_out_dim
            )
            self.pred_v_inter = MLPProjectionHead(
                in_dim=embed_dim, hidden_dim=proj_hidden_dim, out_dim=proj_out_dim
            )
        ...
        if self.ftmode == "multimodal":
            if head_type == "linear":
                ...
                self.mlp_head = nn.Sequential(
                    nn.Linear(proj_out_dim * 2, head_dim),
            ...
            if head_type == "linear":
                ...
                self.mlp_head = nn.Sequential(
                    nn.Linear(proj_out_dim, head_dim),
                    nn.SyncBatchNorm(head_dim),
                    nn.ReLU(inplace=True),
                    nn.Linear(head_dim, label_dim),
                )
