...
## Data definition
parser.add_argument(
    "--dataset", type=str, default="VGGSound", help="name of dataset definition"
)
...
parser.add_argument(
    ...
    default=False,
    help="weight sampling for class balance ex) 'bal'",
)

parser.add_argument(
    "--num_mel_bins", type=int, default=128, help="number of mel bins of spectrogram"
)
...
parser.add_argument(
    ...
    type=lambda x: bool(distutils.util.strtobool(x)),
    default=False,
    help="if use balance sampling",
)
parser.add_argument(
    "--ft_freqm", type=int, default=48, help="frequency mask max length"
)
...
parser.add_argument(
    "--nDataLoaderThread", type=int, default=8, help="Number of loader threads"
)
parser.add_argument(
    "--checkloader",
    dest="checkloader",
    action="store_true",
    help="check the dataloders",
)
...
parser.add_argument(
    "--trainfunc_ft", type=str, default="bceloss", help="Finetuning loss function"
)

## Model definition
parser.add_argument(
    "--model", type=str, default="EquiAV_ft", help="Name of model definition"
)
parser.add_argument(
    "--inter_linear",
    type=bool,
    default=True,
    help="Use the linear head for extracting invariant representation",
)
parser.add_argument(
    "--head_type",
    type=str,
    default="linear",
    choices=["linear", "mlp"],
    help="Head type (linear or mlp)",
)
parser.add_argument(
    "--head_dim", type=int, default=512, help="Dimension for mlp hidden layer"
)
parser.add_argument(
    "--aug_size_a",
    type=int,
    default=21,
    help="Dimension for data augmentation parameters",
)
parser.add_argument(
    "--aug_size_v",
    type=int,
...
parser.add_argument(
    ...
    type=float,
    default=0.1,
    help="drop_path value of the finetuning model",
)
parser.add_argument(
    "--drop_out", type=float, default=0, help="drop_out value of the finetuning model"
)
...
parser.add_argument(
    ...
    type=bool,
    default=False,
    help="Freeze base network without MLP during training",
)
parser.add_argument(
...
parser.add_argument(
    "--weight_decay", type=float, default=1e-5, help="Weight decay in the optimizer"
)
...
parser.add_argument(
    "--start_lr", type=float, default=2e-7, help="start point of learning rate"
)
parser.add_argument(
    "--final_lr", type=float, default=1e-6, help="final point of learning rate"
)

# Scheduler
parser.add_argument(
    "--scheduler", type=str, default="warmupcos", help="Learning rate scheduler"
)
parser.add_argument(
    "--warmup_epoch", type=int, default=3, help="warmup epoch for cosine lr scheduler"
)

## Load and save
parser.add_argument(
    "--save_path", type=str, default="/exp", help="Path for model and logs"
)
parser.add_argument(
    "--model_save_freq", type=int, default=2, help="Frequency of saving model weight"
)
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="pretrained model weights for finetuning",
)
...
args.train_list = f"/home/bethge/twiedemer43/code/avd/models/equiav/datasets/dataprep/{args.dataset}/train.json"
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Initialise dataset
    verbose_print(args.gpu, "\n=================Load Dataset=================")
    DataSet = importlib.import_module("datasets.AudioVisual").__getattribute__(
        "MainDataset"
    )

    norm_stats = {
        "AudioSet_2M": [-4.346, 4.332],
        "AudioSet_20K": [-4.346, 4.332],
        "VGGSound": [-4.956, 4.486],
    }
    ...
    audio_conf = {
        ...
        "im_res": 224,
        "nmels": 128,
        "ft_freqm": args.ft_freqm,
        "ft_timem": args.ft_timem,
        "mixup": args.mixup,
        "noise": True,
    ...
    train_dataset = DataSet(
        dataset_file_name=args.train_list, audio_conf=audio_conf, **vars(args)
    )
    verbose_print(args.gpu, "Load train dataset")
    val_dataset = DataSet(
        dataset_file_name=args.verify_list, audio_conf=val_audio_conf, **vars(args)
    )
    ...
    if args.distributed:
        if args.bal:
            verbose_print(
                args.gpu, "balanced sampler is being used in distributed setting"
            )
            ...
            verbose_print(
                args.gpu, "balanced sampler is not used in distributed setting"
            )
            train_sampler = torch.utils.data.distributed.DistributedSampler(
                train_dataset, rank=args.gpu, drop_last=True
            )

        val_sampler = torch.utils.data.distributed.DistributedSampler(
            val_dataset, rank=args.gpu, drop_last=True
        )

    else:
        if args.bal:
            verbose_print(
                args.gpu, "balanced sampler is being used in not distributed setting"
            )
            samples_weight = np.loadtxt(weight_file, delimiter=",")
            ...
            verbose_print(
                args.gpu, "balanced sampler is not used in not distributed setting"
            )
            train_sampler = None
    ...
    if args.pretrained_model is not None:
        ...
        verbose_print(
            args.gpu, f"Model {args.pretrained_model} loaded from previous state!"
        )

    else:
        verbose_print(
            args.gpu, "Note you are finetuning a model without any finetuning."
        )
    ...
    for epoch in range(epoch, args.max_epoch + 1):
        ...
        with torch.no_grad():
            val_stats, val_loss = trainer.train_network(
                val_loader, evalmode=True, epoch=epoch
            )

            mAUC = np.mean([stat["auc"] for stat in val_stats])
            metric = (
                np.mean([stat["AP"] for stat in val_stats])
                if args.main_metrics == "mAP"
                else val_stats[0]["acc"]
            )
            ...
            if args.gpu == 0:
                ...
                print(
                    ...
                    "{}: EP {:d}, TLOSS {:.5f}, VLOSS {:.5f}\n".format(
                        args.save_path, epoch, train_loss, val_loss
                    ),
                )
                ...
                log_stats = {
                    f"eval_{args.main_metrics}": metric,
                    "eval_mAUC": mAUC,
                ...
                if metric > best_metric:
                    ...
                    print(
                        time.strftime("%Y-%m-%d %H:%M:%S"),
                        f"Saving best metric model with Epoch {epoch} / {args.main_metrics} : {metric:.6f}",
                    )
                    trainer.saveParameters(
                        args.model_save_path + f"/model_bestMetric_ft.pth"
                    )
                ...
                if best_loss > val_loss:
                    ...
                    print(
                        time.strftime("%Y-%m-%d %H:%M:%S"),
                        f"Saving best loss model with Epoch {epoch}",
                    )
                    trainer.saveParameters(
                        args.model_save_path + f"/model_bestLoss_ft.pth"
                    )

                if epoch % args.model_save_freq == 0:
                    print(
                        time.strftime("%Y-%m-%d %H:%M:%S"),
                        "Saving model {:d}".format(epoch),
                    )
                    trainer.saveParameters(
                        args.model_save_path + f"/model_{epoch}_ft.pth"
                    )
...
def main():
    ...
    while os.path.exists(args.save_path):
        if not "try" in args.save_path:
            ...
            new_path = args.save_path.replace(
                args.save_path.split("_")[-1], f"try{i_try}"
            )
